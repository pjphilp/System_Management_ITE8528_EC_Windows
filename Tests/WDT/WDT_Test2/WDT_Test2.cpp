...
//
//    Description:
//      Basic test of the ITE8528 WDT using the minutes mode. Shows use of
...
{
   uint8_t         Minutes = 1;
   WINSYS_ERROR    Status = WDT_SetMinutesCounter( Minutes );

   if ( Status == STATUS_SUCCESS )
       {
          Status = WDT_Start( MINUTE_MODE_ENUM );
          Sleep( ( ( ( Minutes + 1 ) * 60 ) * 1000 ) );         // sleep for 1 minute longer than WDT is set for
